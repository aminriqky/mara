...
class FloodController extends \TCG\Voyager\Http\Controllers\Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->ketinggian > 60 && $request->durasi > 60) {
            foreach ($people as $key) {
                Mail::to($key->email)->send(new MailFlood($key->name, $request->ketinggian,
                    $request->durasi, $request->nama_lokasi, $request->deskripsi));
            }
        }
    ...
    //Dimatikan agar bisa di costum inputannya di controller sendiri
